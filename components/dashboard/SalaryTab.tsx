...
export default function SalaryTab({ data, classes, isAdmin }: Props) {
  ...
  return (
    <div className="p-8">
      ...
      {/* FILTER */}
      <div className="flex flex-wrap gap-4 mb-6 text-slate-800">
        {isAdmin && (
          <input
            ...
            {isAdmin && (
              <>
                ...
                <div className="flex gap-2 mt-2">
                  <button onClick={()=>handleAdmin("approved")} className="flex-1 bg-green-500 text-white p-2 rounded hover:bg-green-600">Duyệt</button>
                  <button onClick={()=>handleAdmin("rejected")} className="flex-1 bg-red-500 text-white p-2 rounded hover:bg-red-600">Từ chối</button>
                </div>
              </>
            )}

            <button onClick={()=>setSelected(null)} className="w-full mt-4 border p-2 rounded hover:bg-slate-200">Đóng</button>
          </div>
        </div>
      )}
